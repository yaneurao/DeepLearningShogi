...
`a=1`では従来の計算経路を維持します。`a<1`かつ勾配蓄積時は、シャッフル後の更新対象の
教師valueから重み平均だけをC++で先読みします。特徴量の再生成やモデルの事前実行は行いません。
policyとvalueを合わせた逆伝播はミニバッチごとに1回で、value勾配用の追加バッファは不要です。
ミニバッチの入力や計算グラフを蓄積数分保持することもありません。キャッシュ利用時は
重み先読みにもキャッシュの読み出しが発生します。

重み付きvalue lossと勾配蓄積を併用する場合は、更新後にネイティブ拡張の再ビルドが必要です。
古い拡張のままなら学習開始前にエラーと再ビルド手順を表示します。
