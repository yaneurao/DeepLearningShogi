...
def main(*argv):
    ...
    def collect_large_batch_bn_stats(forward_model, loader, index_batches):
        ...
        stats = {
            m: {
                'sum': torch.zeros(m.num_features, device=device, dtype=torch.float32),
                'sum_sq': torch.zeros(m.num_features, device=device, dtype=torch.float32),
                'count': 0,
            }
            for m in modules
        }
        ...
        def make_hook(module):
            def hook(_module, inputs):
                ...
                s = stats[module]
                s['sum'] += x.sum(dim=dims)
                s['sum_sq'] += (x * x).sum(dim=dims)
                s['count'] += x.numel() // x.size(1)
            return hook

        for m in modules:
            handles.append(m.register_forward_pre_hook(make_hook(m)))

        forward_model.train()
        bn_states = set_batch_norm_training(modules, False)
        try:
            with torch.no_grad():
            ...
            restore_batch_norm_training(bn_states)
        ...
        for m, s in stats.items():
            ...
            collected[m] = (mean, torch.clamp(var, min=0))
        return collected
