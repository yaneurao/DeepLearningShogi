...
cdef extern from "python_module.h" nogil:
    ...
    size_t __load_hcpe3(const string& filepath, bool use_average, double a, double temperature, size_t& len, double policy_mix)
    ...
    double __hcpe3_value_weight_mean(const size_t len, const char* ndindex, double minimum) except +
...
def load_hcpe3(str filepath, bool use_average, double a, double temperature, double policy_mix=1.0):
    if not 0.0 <= policy_mix <= 1.0:
        raise ValueError("policy_mix must be between 0 and 1")
    cdef size_t len = 0
    cdef size_t size = __load_hcpe3(filepath.encode(locale.getpreferredencoding()), use_average, a, temperature, len, policy_mix)
    return size, len
...
def hcpe3_value_weight_mean(np.ndarray[np.uint64_t, ndim=1, mode='c'] ndindex, double minimum):
    return __hcpe3_value_weight_mean(len(ndindex), ndindex.data, minimum)
